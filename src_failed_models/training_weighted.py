...
# Compute sample weights based on recency of phase_out
# Note: dates were converted to "days since min" earlier; reconstruct datetimes using date_mins
todays_date = pd.to_datetime("today")
phase_out_datetime = date_mins["phase_out"] + pd.to_timedelta(X_df["phase_out"], unit="D")
months_old = (todays_date - phase_out_datetime).dt.days / 30
months_old = months_old.clip(lower=0)
lambda_ = 0.2
sample_weight = np.exp(-lambda_ * months_old.values.astype(np.float32))

# ------------------------------
# Train/val split
X_features = X_df.to_numpy(dtype=np.float32)
X_train, X_val, y_train, y_val, w_train, w_val = train_test_split(
    X_features, y, sample_weight, test_size=0.2, random_state=42
)

# ------------------------------
# XGBoost: quantile regression to penalize under-prediction more
# NOTE: requires xgboost >= 1.7 for "reg:quantileerror".
quantile_alpha = 0.65  # >0.5 means preferring over-production vs under-production

dtrain = xgb.DMatrix(X_train, label=y_train, weight=w_train)
dval = xgb.DMatrix(X_val, label=y_val, weight=w_val)

params = {
	"num_boost_round": 885, # best trees from eric_training.py
