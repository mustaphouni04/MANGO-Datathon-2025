...
class MixedInputNet(nn.Module):
    ...
    def __init__(self, embedding_dim, num_numerical_features, cat_embedding_sizes):
        ...
        # --- 3. Combined Head ---
        # Input: 128 (from img) + num_numerical + total_cat_embed_dim
        combined_input_dim = 128 + num_numerical_features + total_cat_embed_dim
    ...
    def forward(self, img_emb, num_data, cat_data):
        ...
        for i, embed_layer in enumerate(self.cat_embeddings):
            # cat_data[:, i] selects the i-th categorical feature for all samples
            x_cats.append(embed_layer(cat_data[:, i]))
        
        x_cats = torch.cat(x_cats, dim=1)
        
        # Combine all features
        # Note: num_data is passed through directly
        combined = torch.cat([x_img, num_data, x_cats], dim=1)
...
# --- 5. Training ---
quantile_alpha = 0.8 # Same as your XGBoost
criterion = QuantileLoss(alpha=quantile_alpha)
optimizer = optim.Adam(model.parameters(), lr=1e-3)

num_epochs = 800 # NNs can take longer, but we have early stopping
early_stopping_rounds = 50 # User had 200, but for NNs 20-30 is more common
