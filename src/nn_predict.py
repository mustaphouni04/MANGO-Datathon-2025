...
class MixedInputNet(nn.Module):
    def __init__(self, embedding_dim, num_numerical_features, cat_embedding_sizes):
        ...
        self.img_branch = nn.Sequential(
            nn.Linear(embedding_dim, 256), nn.ReLU(), nn.BatchNorm1d(256),
        ...
        self.combiner = nn.Sequential(
            nn.Linear(combined_input_dim, 512), nn.ReLU(), nn.BatchNorm1d(512),
            nn.Dropout(0.5), nn.Linear(512, 256), nn.ReLU(),
            nn.BatchNorm1d(256), nn.Dropout(0.35), nn.Linear(256, 1)
        )
