...
train_loader = DataLoader(
    ...
    shuffle=True, collate_fn=collate_fn, pin_memory=use_cuda, num_workers=2, persistent_workers=True
)
val_loader = DataLoader(
    ProductDataset(X_val, y_val), batch_size=8,
    shuffle=False, collate_fn=collate_fn, pin_memory=use_cuda, num_workers=2, persistent_workers=True
)
...
seq_example = seq_example.to(device)
with torch.no_grad():
    pred = model(seq_example, length_example)
pred = pred.detach().cpu() * target_std + target_mean  # denormalize
